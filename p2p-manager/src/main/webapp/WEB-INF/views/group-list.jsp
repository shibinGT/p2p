				...
				<div class="table-responsive">
					<div id="sample-table-2_wrapper" class="dataTables_wrapper"
						...
						<div class="row">
							...
							<div class="col-sm-2">
								<a href="${ctx }/group/toAdd" class="btn btn-primary btn-sm"
									group="button">新增分组</a>
							</div>
							...
							<input type="button" class="btn btn-primary btn-sm"
                                    group="button" value="批量删除" id="delgroup"> 
                               
                            </div>
                            
						</div>
						<form action="${ctx }/group/batchDel" method="post" id="delgroupForm">
						<table id="sample-table-2"
							class="table table-striped table-bordered table-hover dataTable"
							aria-describedby="sample-table-2_info">
							<thead>
								<tr group="row">
									<th class="center sorting_disabled" group="columnheader"
										rowspan="1" colspan="1" aria-label="" style="width: 96px;"><label>
									<input type="checkbox"  class="ace"  id="choose_group" > <span class="lbl"></span>
									</label></th>
									...
									<th class="sorting" group="columnheader" tabindex="0"
										...
										style="width: 179px;">组内成员</th>
									<th class="sorting" group="columnheader" tabindex="0"
										aria-controls="sample-table-2" rowspan="1" colspan="1"
										aria-label="Price: activate to sort column ascending"
										style="width: 179px;">分组角色</th>
									<th class="sorting_disabled" group="columnheader" rowspan="1"
								...
								<c:forEach items="${pageInfo.list }" var="group">
									<tr class="odd">
										<td class="center  sorting_1"><label> <input
												...
												type="checkbox" class="ace" name="lists" value="${group.groupId }"> <span class="lbl"></span>
												
										</label></td>
										<td class=" "><a href="#">${group.groupCode }</a></td>
										<td class=" ">${group.groupDesc}</td>
										<td>
											<c:forEach items="${group.userNames}" var="userName">
												${userName }、&nbsp;&nbsp;&nbsp;
											</c:forEach>
										</td>
										<td>
											<c:forEach items="${group.roleCodes}" var="roleCode">
												${roleCode }、&nbsp;&nbsp;&nbsp;
											</c:forEach>
										</td>
										<td class=" ">
											<div
												...
												<shiro:hasPermission name="user:*">
													<!-- 分配权限 -->
													<a class="green" href="${ctx }/group/toUpdate?id=${group.groupId}"> <i
														class="icon-pencil bigger-130"></i>
													</a>
													  <!-- 	删除 -->
													<a class="red del_group" href="javascript:void(0)"  groupId="${group.groupId }"> <i  
					...
					<page:pager url="${ctx }/group/list" pageInfo="${pageInfo }" />
...
// 		删除分组链接
    $("a.del_group").click(function(){
        if(confirm("是否要删除吗?")){
            ...
            window.location.href=ctx+"/group/delById?id="+id;
        }
    });
