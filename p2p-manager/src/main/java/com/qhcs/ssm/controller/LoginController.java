...
public class LoginController {
	...
	public String main(Employee employee, Model model) {
		...
		// redis取出金额人数等数据
		// 若果redis里面没有数据，则设置进去
		if (redis.get("totalMoney") == null || redis.get("todayMoney") == null) {
			managerIndexService.setTotalInvestMoney();
		}
		String totalMoney = redis.get("totalMoney");
		String todayMoney = redis.get("todayMoney");
		System.out.println(todayMoney + "------\n\n\n---------" + totalMoney);
		// 取出注册人数统计数据
		String jsonStr = managerIndexService.getRegNumByDate();
		// 近期应还款项目
		List<Project> projects = managerIndexService.getRepaymentList();
